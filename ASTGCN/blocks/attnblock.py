...
class SpatialAttention(nn.Module):
    ...
    def __init__(self, params):
        ...
        '''self.n_verticies = params.n_verticies
        ...
        self.w1 = nn.Parameter(torch.randn(size=self.params.w1_size))
        self.w2 = nn.Parameter(torch.randn(size=self.params.w2_size))
        self.w3 = nn.Parameter(torch.randn(size=self.params.w3_size))
        self.vs = nn.Parameter(torch.randn(size=self.params.vs_size)) 

        self.bias = nn.Parameter(torch.empty(size=self.params.s_bias_size))
    ...
    def forward(self, x):
        ...
        inner_prod = torch.bmm(x_w1_w2, w3_x_transposed) 

        S_prime = self.softmax(self.vs.unsqueeze(0) * self.sigmoid(inner_prod + self.bias.unsqueeze(0)))
...
class TemporalAttention(nn.Module):
    ...
    def __init__(self, params):
        ...
        self.u1 = nn.Parameter(torch.randn(size=self.params.u1_size))
        self.u2 = nn.Parameter(torch.randn(size=self.params.u2_size))
        self.u3 = nn.Parameter(torch.randn(size=self.params.u3_size))
        self.vt = nn.Parameter(torch.randn(size=self.params.vt_size)) # learnables

        self.bias = nn.Parameter(torch.empty(size=self.params.t_bias_size))
    ...
    def forward(self, x):
        ...
        # btn, bnt -> btt
        inner_product = torch.bmm(xT_u1_u2, u3_x) 

        E_prime = self.softmax(self.vt.unsqueeze(0) * self.sigmoid(inner_product + self.bias.unsqueeze(0)))
